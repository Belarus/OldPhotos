  ...
  L.DivIcon.ArrowCircle = L.DivIcon.extend({
    ...
    _createSVG: function () {
      ...
      if (this.options.rotation != null) {
        group = `<g transform="rotate(${this.options.rotation}, ${this.iconAnchor.x}, ${this.iconAnchor.y})">` +
        `${this._createArrow()}${this._createCircle()}</g>`;
      } else {
        group = `<g transform="rotate(0, ${this.iconAnchor.x}, ${this.iconAnchor.y})">` +
        `${this._createCircle()}</g>`;
      }
